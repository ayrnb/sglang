...
"""Inference-only WeLMV4 MoE model compatible with HuggingFace weights."""
...
def sigmoid_routing_function(
    ...
    # if softmax, then use softmax routing function
    scores = torch.sigmoid(gating_output).type_as(gating_output)
...
class WeLMV4SparseMoeBlock(nn.Module):
    ...
    def __init__(
        ...
        if config.shared_expert_intermediate_size > 0:
            self.shared_expert = WeLMV4MoeMLP(
                hidden_size=config.hidden_size,
        ...
        has_shared_expert_gate = getattr(
            ...
            True)  # default to true since WeLMV4 always has it
...
class WeLMV4YarnScalingRotaryEmbedding(RotaryEmbedding):
    def __init__(
        self,
        head_size: int,
        rotary_dim: int,
    ...
    if rope_scaling is None:
        ...
        if scaling_type == "linear":
            ...
            if max_position != int(original_max_position * scaling_factor):
                raise ValueError(f"max_position ({max_position}) != original_max_position ({original_max_position}) * scaling_factor ({scaling_factor})")
            extra_kwargs = {
                k: v
            ...
            rotary_emb = WeLMV4YarnScalingRotaryEmbedding(
                head_size,
                rotary_dim,
...
class WeLMV4Attention(nn.Module):
    ...
    def forward(
        ...
        if self.layer_idx in self.kv_mirror_imitated_layers:
            ...
            if forward_batch.enable_kv_mirror and forward_batch.forward_mode == ForwardMode.EXTEND and not hasattr(forward_batch, 'custom_last_index'):
                forward_batch.custom_last_index = (torch.cumsum(forward_batch.extend_seq_lens, dim=0) - 1).clone()
        ...
        if qk_nope_head_dim > 0:
            ...
            if forward_batch.enable_kv_mirror and forward_batch.forward_mode == ForwardMode.EXTEND and self.layer_idx in self.kv_mirror_layers:
...
class WeLMV4DecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.self_attn = WeLMV4Attention(
        ...
        # WeLMV4 all layers are sparse (include nextn layers)
    ...
    def forward(
        ...
        # )
        if forward_batch.enable_kv_mirror and forward_batch.forward_mode == ForwardMode.EXTEND and self.layer_id == self.kv_mirror_layers[-1]:
...
class WeLMV4Model(nn.Module):
    def __init__(
        self,
        config: PretrainedConfig,
        quant_config: Optional[QuantizationConfig] = None,
        prefix: str = "",
        decoder_layer_type: type[nn.Module] = WeLMV4DecoderLayer,
        alt_stream: Optional[torch.cuda.Stream] = None,
    ) -> None:
        super().__init__()
        self.config = config
        ...
        # Use the provided decoder layer type or default to WeLMV4DecoderLayer
        decoder_layer_type = decoder_layer_type or WeLMV4DecoderLayer
            ...
            if len(self.oe_grams) > 0 and forward_batch.n_gram_input_ids is not None:
                ...
                for g in range(1, max(self.oe_grams)):
                    ...
                    if gram_tensor is not None:
                        input_ids_ngram_tmp = input_ids_ngram_tmp + gram_tensor * (self.vocab_size ** g)
...
class WeLMV4MoeForCausalLM(nn.Module):
    ...
    def get_num_n_gram(cls, config) -> int:
        ...
        Returns max(oe_grams) from config, or 0 if not using over encoding.
        """
